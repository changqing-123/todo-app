...
export default function ToDOItem({ title, data, onChange, onDelete }: IProps) {
  const { hours, minutes, seconds } = formateTime(data.duration ?? 0)
  ...
  const onPlay = async id => {
    try {
      ...
      console.log('res', res)
      Taro.navigateTo({
        url: `/packageTimer/pages/timer/index?id=${data.id}&start_time=${startTime}&timer_type=${data.timer_type}&title=${data.title}&duration=${data.duration}`
      })
    } catch (error) {
      console.error(error)
    }
  ...
  return (
    <View className={`${styles.container} ${data.completed ? styles.completed : ''}`}>
      ...
      <View className={styles.text}>
        ...
        <Text className={styles.periods}>
          {data.duration ? renderDuration({ hours, minutes, seconds }) : null}
        </Text>
      </View>
